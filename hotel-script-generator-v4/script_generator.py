#!/usr/bin/env python3
"""
Hotel YouTube Script Generator V4
Designed for Kaggle GPU + Hugging Face Transformers.

V4 generates the script in separate sections with tighter section limits.
Target: about 1,350-1,450 words / roughly 9-10 minutes.
"""
...
SECTIONS = [
    ("INTRO", 70, 100, """Write a short engaging opening for this hotel review.
...
claims."""),
    ("OVERVIEW", 100, 130, """Explain what the hotel is, its property class, overall
guest rating and review count, and the strongest supported general impressions.
Use only the supplied data."""),
    ("LOCATION", 120, 160, """Explain the hotel's location and nearby landmarks,
...
data. Include the supplied distances naturally. Do not invent anything."""),
    ("ROOMS", 160, 210, """Describe every useful room option in the supplied data.
...
when available. Compare the options naturally without inventing differences."""),
    ("AMENITIES", 150, 200, """Explain the useful hotel amenities and facilities.
...
supported amenities. Mention limitations such as no onsite parking when supported."""),
    ("DINING_AND_POLICIES", 130, 170, """Cover breakfast and restaurant availability
...
include facts actually present in the data."""),
    ("REVIEWS", 190, 240, """Give a balanced review summary. Start with the overall
...
into a general claim. Explain what the review pattern suggests."""),
    ("VERDICT", 100, 140, """Give a balanced conclusion: who this hotel may suit,
...
def main():
    ...
    # Safety cap: keep the finished narration under the requested 10-minute range.
    # Trim only at a sentence boundary and never below 1,300 words.
    words_list = re.findall(r"\S+", script)
    if len(words_list) > 1500:
        candidate = " ".join(words_list[:1500])
        boundary = max(candidate.rfind(". "), candidate.rfind("! "), candidate.rfind("? "))
        if boundary >= 1300 * 4:
            script = candidate[:boundary + 1].strip()
    ...
    if words < 1300:
        print("WARNING: Below the 9-10 minute target.")
