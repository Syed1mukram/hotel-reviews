...
class TimelineBuilder:
    ...
    def select_visual(
        ...
        # Known script concepts always take priority. Only use the old
        # generator when this sentence has no matching script keyword.
        if not prompt:
        ...
        # Clean transcript filler from fallback queries.
        if prompt:
            filler = {
                "mind", "there", "here", "your", "the", "and", "or",
                "with", "for", "some", "this", "that", "these", "those",
                "yourself", "trebo", "c", "plaza"
            }
            cleaned = [w for w in prompt.split() if w.lower() not in filler]
            if cleaned:
                prompt = " ".join(cleaned)
