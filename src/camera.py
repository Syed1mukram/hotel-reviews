...
class Camera:
    ...
    def get_transform(
        ...
        # Very subtle fixed-pixel camera movement.
        # Keeps composition stable regardless of source image size.
        PAN_X = 30.0
        PAN_Y = 20.0

        if motion == "left":

            tx = max_x / 2.0 + PAN_X * (1.0 - progress)

        elif motion == "right":

            tx = max_x / 2.0 + PAN_X * progress

        elif motion == "up":

            ty = max_y / 2.0 + PAN_Y * (1.0 - progress)

        elif motion == "down":

            ty = max_y / 2.0 + PAN_Y * progress
